...
WebSocket upgrades on `/terminal/*` check the `Origin` header against the request's `Host` header. Browsers send Origin on WebSocket handshakes and it cannot be forged by cross-origin pages. This blocks cross-site WebSocket CSRF. Implemented in `checkWebSocketOrigin()` in `security.go`.
...
`/terminal/*` is accessible via GET (no CSRF required) because the browser needs to load ttyd's HTML, JavaScript, and WebSocket. However, ttyd sessions only appear in the instance table after a POST to `/connect/` (protected by CSRF middleware and POST enforcement).
